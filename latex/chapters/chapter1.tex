...
an error tolerance of 5 cm. Many o rganic farmers are not equipped
...
(Figure). These rods make contact with the crop stems and their
...
channels. Therefore, conversion to an alternate color-space is
necessary for plant identification. 

An approach proposed by Meyer et al. (1998) demonstrates a contrast
filter known as Excess Green (ExG) calculated using the following
equation:
...
\begin{equation}
  ...
  \label{eq:bppd}
\end{equation}
where Hmin=45 lower hue threshold (yellow-green), Hmax=105 upper hue
threshold (blue-green).
...
\section{Results}
\begin {table}
  \caption{RMSE and 95th percentile with respect to travel speed.}
  \centering
...
\begin {table}
  \caption{RMSE and 95th percentile with respect to crop stage.}
  \centering
  \begin{tabular}{l*{6}{c}r}
    Team              & P & W & D & L & F  & A & Pts \\
    \hline
